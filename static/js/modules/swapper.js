define(['modules/helpers/loader','modules/helpers/events','modules/globals', 'easel','tween'], function(loader, events, globals) {
    ...
    Swapper.prototype.move = function(movement){
        ...
        var swapper = this;
        if (movement == 0){
            createjs.Tween.get(this).to({x: this.x, y: this.y - globals.blockSize}, 150,createjs.Ease.quartInOut)
                .call(function() {
                    swapper.moving=false;
                })
        }
        else if(movement == 1){
            createjs.Tween.get(this).to({x: this.x + globals.blockSize, y: this.y}, 150,createjs.Ease.quartInOut)
                .call(function() {
                    swapper.moving=false;
                });
        }
